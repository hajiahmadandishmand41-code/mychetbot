...
Nara's public site currently documents `auto/bynara` as a routing alias and lists `Agnes 2.0 Flash` in the Free plan. Model availability and aliases can change over time, so production configuration should use a currently supported model for the account/plan.
