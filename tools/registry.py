...
register(Tool("wifi_capabilities", "تشخیص قابلیت‌ها و محدودیت‌های قانونی Wi-Fi Audit", {}, wifi_tool.capability_detection))
register(Tool("wifi_info", "اطلاعات اتصال Wi-Fi فعلی", {}, wifi_tool.wifi_info))
register(Tool("wifi_scan", "اسکن غیرفعال شبکه‌های Wi-Fi اطراف با اطلاعات امنیتی", {}, wifi_tool.wifi_scan))
register(Tool("wifi_diagnostics", "تشخیص اتصال، مسیر، DNS و دسترسی اینترنت Wi-Fi", {}, wifi_tool.network_diagnostics))
register(Tool("wifi_security_report", "گزارش امنیتی passive برای شبکه Wi-Fi فعلی", {}, wifi_tool.security_report))
