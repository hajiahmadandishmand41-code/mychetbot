...
export default function ChatShell() {
  ...
  const loadSession = useCallback(async (id: string) => {
    ...
    if (!response.ok) {
      setError(await apiError(response, "تغییر گفت‌وگو انجام نشد."));
      return;
    }
    setSessionId(id);
    const history = await fetch("/api/history", { cache: "no-store" });
    if (!history.ok) {
      setError(await apiError(history, "تاریخچه گفت‌وگو بارگذاری نشد."));
      return;
    }
    const payload = (await history.json()) as { messages?: Message[] };
    setMessages((payload.messages ?? []).filter((item) => item.role === "user" || item.role === "assistant"));
  ...
  const newChat = useCallback(async () => {
    ...
    if (!response.ok) {
      setError(await apiError(response, "چت جدید ساخته نشد."));
      return;
    }
    const data = (await response.json()) as { sessionId?: string };
    if (!data.sessionId) {
      setError("چت جدید شناسه نشست دریافت نکرد.");
      return;
    }
    setSessionId(data.sessionId);
    setMessages([]);
  ...
  const clearMemory = async () => {
    ...
    if (!response.ok) {
      setError(await apiError(response, "پاک‌سازی حافظه انجام نشد."));
      return;
    }
    setMessages([]);
  };
    ...
    <main className={`app-shell ${sidebarOpen ? "sidebar-visible" : ""}`}>
      <aside className="sidebar" aria-label="فهرست گفت‌وگوها">
        <div className="brand-row">
          ...
          <div><strong>{APP_NAME}</strong><span>دستیار هوش مصنوعی</span></div>
        </div>
      ...
      <section className="chat-panel">
        <header className="topbar">
          ...
          <div className="top-title">
            <strong>{currentConversation?.title ?? APP_NAME}</strong>
            <span>دستیار هوش مصنوعی یکپارچه</span>
          </div>
        ...
        {mobileMenuOpen && (
          <>
            <button className="menu-backdrop" aria-label="بستن منو" onClick={() => setMobileMenuOpen(false)} />
            <div className="mobile-menu" role="menu" aria-label="گزینه‌های چت">
              <button role="menuitem" onClick={() => { setMobileMenuOpen(false); setSidebarOpen(false); }}>{icon("back")} <span>برگشت به چت</span></button>
              <button role="menuitem" onClick={() => void newChat()}>{icon("plus")} <span>چت جدید</span></button>
              <button role="menuitem" onClick={() => { setDark((value) => !value); setMobileMenuOpen(false); }}>{icon(dark ? "sun" : "moon")} <span>{dark ? "حالت روشن" : "حالت تاریک"}</span></button>
              <button role="menuitem" onClick={() => { setMobileMenuOpen(false); void clearMemory(); }}>{icon("trash")} <span>پاک‌کردن چت</span></button>
            </div>
          </>
        )}
        ...
        <div className="message-scroll">
          {messages.length === 0 ? (
            <div className="empty-state">
              ...
              <h1>سلام! 👋</h1>
              <p>من {APP_NAME} هستم.<br />دستیار هوش مصنوعی شما. چطور کمکتان کنم؟</p>
              <div className="suggestions">
                <button onClick={() => setInput("درباره هوش مصنوعی برایم تحقیق کن.")}>درباره یک موضوع تحقیق کن</button>
                <button onClick={() => setInput("آخرین اطلاعات مهم امروز را برایم پیدا کن.")}>اطلاعات جدید را پیدا کن</button>
            ...
            <div className="messages">
              {messages.map((message, index) => (
                <div key={`${index}-${message.role}`} className={`message-row ${message.role}`}>
                  <div className="avatar">{message.role === "user" ? "ش" : "س"}</div>
                  <article className="message-card">
                    ...
                    <div className="message-actions">
                      <button onClick={() => void navigator.clipboard.writeText(message.content).then(() => { setCopied(index); window.setTimeout(() => setCopied(null), 1500); })} aria-label="کپی پیام">{icon("copy")} {copied === index && <span>کپی شد</span>}</button>
              ...
              {loading && (
                <div className="message-row assistant">
                  ...
                  <div className="message-card generating"><div className="message-meta">{APP_NAME}</div><div className="typing"><span /><span /><span /><em>در حال فکر کردن…</em></div></div>
                </div>
              )}
              <div ref={endRef} />
            </div>
        ...
        <form className="composer-wrap" onSubmit={(event: FormEvent) => { event.preventDefault(); void send(); }}>
          ...
          <div className="composer-note">{APP_NAME} · دستیار هوش مصنوعی شخصی شما</div>
        </form>
      </section>
    </main>
  );
